                ...
                <table className="data-table">
                    ...
                    {applyFilters ( data ).map ( (item , index) => (
                        <tr key={index}>
                            ...
                            <td><Link to={`/reference/Модель%20техники/${item.model_name}`}>{item.model_name}</Link>
                            </td>
                            <td><Link
                                to={`/reference/Модель%20двигателя/${item.engine_model_name}`}>{item.engine_model_name}</Link>
                            </td>
                            <td>{renderCell ( item.engine_serial_number )}</td>
                            <td><Link
                                to={`/reference/Модель%20трансмиссии/${item.transmission_model_name}`}>{item.transmission_model_name}</Link>
                            </td>
                            <td>{renderCell ( item.transmission_serial_number )}</td>
                            <td><Link
                                to={`/reference/Модель%20ведущего%20моста/${item.driving_axle_model_name}`}>{item.driving_axle_model_name}</Link>
                            </td>
                            <td>{renderCell ( item.driving_axle_serial_number )}</td>
                            <td><Link
                                to={`/reference/Модель%20управляемого%20моста/${item.steering_axle_model_name}`}>{item.steering_axle_model_name}</Link>
                            </td>
