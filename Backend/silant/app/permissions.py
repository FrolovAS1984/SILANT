...
class DetailedViewPermission(permissions.BasePermission):
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
                    request.user.is_client or request.user.is_service_company or request.user.is_manager)
...
class MaintenancePermission(permissions.BasePermission):
    """
    Определяет разрешения для модели ТО в зависимости от роли пользователя.
    """
...
class ComplaintsPermission(permissions.BasePermission):
    """
    Определяет разрешения для модели Рекламаций в зависимости от роли пользователя.
    """
...
class IsReferenceAccessible(permissions.BasePermission):
    def has_permission(self, request, view):
        public_directories = ['Модель техники', 'Модель двигателя', 'Модель трансмиссии', 'Модель ведущего моста',
