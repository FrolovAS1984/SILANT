...
class MachineBasicInfoAdmin(admin.ModelAdmin):
    list_display = ['serial_number', 'model_display', 'engine_model_display', 'engine_serial_number',
                    'transmission_model_display', 'transmission_serial_number', 'driving_axle_model_display',
                    'driving_axle_serial_number', 'steering_axle_model_display', 'steering_axle_serial_number']
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name in directory_fields:
            kwargs["queryset"] = References.objects.filter(directory_name=directory_fields[db_field.name])
    ...
    # Менеджеры и суперпользователи могут добавлять, удалять и изменять объекты
...
class MachineAllInfoAdmin(admin.ModelAdmin):
    list_filter = ['machine', 'shipment_date']
    list_display = ['machine', 'supply_contract_number', 'shipment_date', 'consignee', 'operation_address',
                    'configuration', 'client_display', 'service_companies_display']
    ...
    def client_display(self, obj):
        return obj.client.company_name if obj.client and obj.client.company_name else 'None'

    client_display.short_description = 'Клиент'

    def service_companies_display(self, obj):
        return ", ".join([sc.company_name for sc in obj.service_companies.all() if sc.company_name])

    service_companies_display.short_description = 'Сервисная компания'
...
class MaintenanceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('machine', 'maintenance_type', 'date_performed', 'hours_worked', 'order_number', 'order_date',
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "performing_organization":
            kwargs["queryset"] = References.objects.filter(
                Q(directory_name='Организация, проводившая ТО') | Q(name='Self-maintained'))
        elif db_field.name == "maintenance_type":
            kwargs["queryset"] = References.objects.filter(directory_name='Вид ТО')
        return super().formfield_for_foreignkey(db_field, request, **kwargs)
...
class ComplaintsAdmin(admin.ModelAdmin):
    list_display = ('machine', 'failure_date', 'operating_hours', 'failure_node_display', 'failure_description',
                    'restoration_method_display', 'spare_parts_used', 'restoration_date', 'downtime')
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name in directory_mappings:
            kwargs["queryset"] = References.objects.filter(directory_name=directory_mappings[db_field.name])
        return super().formfield_for_foreignkey(db_field, request, **kwargs)
...
admin.site.register(MachineAllInfo, MachineAllInfoAdmin)
